"""
Encryption for BCH-based McEliece cryptosystem.
"""
import numpy as np
import galois
...
def encrypt(msg_bits, G_pub, t, S=None, G_total=None, P=None):
    ...
    Args:
        ...
        G_pub: Public generator matrix
        t: Number of errors to add (t = 2*L for BCH, two errors per block)
    ...
    if S is not None and G_total is not None and P is not None:
        ...
        # Step 3: Add errors (two per block)
        n_total = len(m_S_G)
        block_size = 15
        L = t // 2  # Since t = 2*L
        ...
        # Add exactly 2 errors per block
        for i in range(L):
            block_start = i * block_size
            block_end = (i + 1) * block_size
            # Randomly choose 2 positions in this block
